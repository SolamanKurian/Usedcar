...
export default function EditFuelType() {
  ...
  useEffect(() => {
    if (isConfigured && db && fuelTypeId) {
      fetchFuelTypeData();
      fetchExistingFuelTypes();
    }
  }, [isConfigured, fuelTypeId]);

  const fetchFuelTypeData = async () => {
    try {
      setIsLoading(true);
      const fuelTypeDoc = await getDoc(doc(db!, 'fuelTypes', fuelTypeId));
      
      if (!fuelTypeDoc.exists()) {
        setError('Fuel type not found');
        return;
      }

      const fuelTypeData = fuelTypeDoc.data();
      setFormData({
        name: fuelTypeData.name
      });
    } catch (error) {
      console.error('Error fetching fuel type:', error);
      setError('Failed to load fuel type data');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchExistingFuelTypes = async () => {
    try {
      ...
      const fuelTypesData = fuelTypesSnapshot.docs
        .filter(doc => doc.id !== fuelTypeId) // Exclude current fuel type
        .map(doc => doc.data().name.toLowerCase());
      setExistingFuelTypes(fuelTypesData);
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (existingFuelTypes.includes(formData.name.trim().toLowerCase())) {
      setError('A fuel type with this name already exists');
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(doc(db!, 'fuelTypes', fuelTypeId), fuelTypeData);
      router.push('/admin/fuel-types');
    } catch (error) {
      console.error('Error updating fuel type:', error);
      setError('Failed to update fuel type. Please try again.');
    } finally {
  ...
  if (error && error === 'Fuel type not found') {
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-xl text-gray-600 mb-8 max-w-2xl mx-auto">
              The fuel type you&apos;re looking for doesn&apos;t exist or has been deleted.
            </p>
            <Link
              href="/admin/fuel-types"
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center justify-between flex-1">
                <div>
                  ...
                  <p className="text-gray-600 mt-1">Update fuel type information</p>
                </div>
                <Link
                  href="/admin/fuel-types"
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <input
                      ...
                      placeholder="Enter fuel type name (e.g., Petrol, Diesel, Electric, Hybrid)"
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  <button
                    ...
                    disabled={isSubmitting}
                  >
                    {isSubmitting ? 'Updating...' : 'Update Fuel Type'}
                  </button>
                </div>
              </form>
            </div>
