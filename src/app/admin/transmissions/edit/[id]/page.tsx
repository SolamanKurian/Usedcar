...
export default function EditTransmission() {
  ...
  const transmissionId = params.id as string;
  ...
  useEffect(() => {
    if (isConfigured && db && transmissionId) {
      fetchTransmissionData();
      fetchExistingTransmissions();
    }
  }, [isConfigured, transmissionId]);

  const fetchTransmissionData = async () => {
    try {
      setIsLoading(true);
      const transmissionDoc = await getDoc(doc(db!, 'transmissions', transmissionId));
  ...
  const fetchExistingTransmissions = async () => {
    try {
      ...
      const transmissionsData = transmissionsSnapshot.docs
        .filter(doc => doc.id !== transmissionId) // Exclude current transmission
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(doc(db!, 'transmissions', transmissionId), transmissionData);
      router.push('/admin/transmissions');
    } catch (error) {
      console.error('Error updating transmission:', error);
      setError('Failed to update transmission type. Please try again.');
    } finally {
  ...
  if (error && error === 'Transmission not found') {
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-xl text-gray-600 mb-8 max-w-2xl mx-auto">
              The transmission type you're looking for doesn't exist or has been deleted.
            </p>
            <Link
              href="/admin/transmissions"
          ...
          <div className="mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center justify-between flex-1">
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Edit Transmission Type</h1>
                  <p className="text-gray-600 mt-1">Update transmission type information</p>
                </div>
                <Link
                  href="/admin/transmissions"
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <input
                      ...
                      placeholder="Enter transmission type name (e.g., Manual, Automatic, CVT, Semi-Automatic)"
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  <button
                    ...
                    disabled={isSubmitting}
                  >
                    {isSubmitting ? 'Updating...' : 'Update Transmission Type'}
                  </button>
                </div>
              </form>
            </div>
