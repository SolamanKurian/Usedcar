...
export default function TransmissionsManagement() {
  ...
  const [deletingTransmission, setDeletingTransmission] = useState<string | null>(null);
  ...
  const fetchTransmissions = async () => {
    try {
      ...
      // Sort by name
      transmissionsData.sort((a, b) => a.name.localeCompare(b.name));
  ...
  const handleDeleteTransmission = async (transmissionId: string, transmissionName: string) => {
    if (!confirm(`Are you sure you want to delete the transmission "${transmissionName}"? This action cannot be undone.`)) {
      return;
    }

    try {
      setDeletingTransmission(transmissionId);
      await deleteDoc(doc(db!, 'transmissions', transmissionId));
      setTransmissions(transmissions.filter(transmission => transmission.id !== transmissionId));
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-xl text-gray-600 mb-8 max-w-2xl mx-auto">
              Firebase is not configured. Please set up your environment variables to access transmission management.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex-1 p-4 lg:p-6">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Transmission Types</h1>
                  <p className="text-gray-600 mt-1">Manage transmission types for your products</p>
                </div>
              </div>
              <Link
                href="/admin/transmissions/add"
              ...
              >
                ...
                <span>Add Transmission Type</span>
              </Link>
            </div>
          </div>
          ...
          {loadingTransmissions ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Loading transmission types...</p>
            </div>
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No transmission types found</h3>
              <p className="text-gray-600 mb-6">Get started by adding your first transmission type.</p>
              <Link
                href="/admin/transmissions/add"
                  ...
                  <li key={transmission.id}>
                    <div className="px-6 py-4 flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-medium text-gray-900">{transmission.name}</div>
                          <div className="text-sm text-gray-500">
                            Created {transmission.createdAt.toLocaleDateString()}
                          </div>
                        </div>
                      </div>
                      <div className="flex space-x-2">
                        ...
                        <button
                          onClick={() => handleDeleteTransmission(transmission.id, transmission.name)}
                          disabled={deletingTransmission === transmission.id}
                          className="bg-red-600 hover:bg-red-700 text-white text-sm font-medium py-2 px-3 rounded-md transition-colors duration-200 disabled:opacity-50"
                        >
                          {deletingTransmission === transmission.id ? 'Deleting...' : 'Delete'}
                        </button>
                      </div>
                    </div>
                  </li>
