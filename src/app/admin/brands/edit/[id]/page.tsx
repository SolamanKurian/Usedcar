...
export default function EditBrand() {
  ...
  useEffect(() => {
    if (isConfigured && db && brandId) {
      fetchBrandData();
      fetchExistingBrands();
    }
  }, [isConfigured, brandId]);
  ...
  const fetchExistingBrands = async () => {
    try {
      ...
      const brandsData = brandsSnapshot.docs
        .filter(doc => doc.id !== brandId) // Exclude current brand
  ...
  const validateForm = () => {
    ...
    if (existingBrands.includes(formData.name.trim().toLowerCase())) {
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <input
                      ...
                      placeholder="Enter brand name (e.g., Toyota, Honda, Ford)"
