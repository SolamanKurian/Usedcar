...
export default function EditProduct() {
  ...
  const fetchProductData = async () => {
    try {
      ...
      setFormData({
        ...
        yearOfManufacture: productData.yearOfManufacture.toString(),
        kilometersDriven: productData.kilometersDriven.toString(),
        fuel: productData.fuel || '',
        transmission: productData.transmission || '',
        description: productData.description,
        includedInOffer: productData.includedInOffer || false,
        priorityNumber: productData.priorityNumber ? productData.priorityNumber.toString() : ''
      });
      
             // Convert worker URLs back to R2 URLs for existing images display
       const rawImages = productData.images || productData.imageUrls || [];
       const convertedImages = rawImages.map(convertWorkerUrlToR2);
       setExistingImages(convertedImages);
       console.log('Loaded existing images (converted to R2):', convertedImages);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload new images to R2 if any
      if (newImages.length > 0) {
        const uploadedImageUrls = await uploadImagesToR2(newImages);
        allImageUrls = [...existingImages, ...uploadedImageUrls];
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
            ...
            <div className="flex justify-between h-16">
              <div className="flex items-center space-x-8">
                ...
                <div className="flex space-x-1">
                  <Link href="/admin" className="text-gray-700 hover:text-blue-600 px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200">
                    Dashboard
                  </Link>
                  <Link href="/admin/brands" className="text-gray-700 hover:text-blue-600 px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200">
                    Brands
                  </Link>
                  <Link href="/admin/categories" className="text-gray-700 hover:text-blue-600 px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200">
                    Categories
                  </Link>
                  <Link href="/admin/products" className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-md">
                    Products
                  </Link>
                </div>
              </div>
        ...
        <div className="max-w-7xl mx-auto py-12 sm:px-6 lg:px-8">
          <div className="px-4 py-8 sm:px-0">
            <div className="text-center">
              <div className="mx-auto h-24 w-24 bg-gradient-to-r from-orange-400 to-orange-600 rounded-full flex items-center justify-center mb-6">
                <svg className="h-12 w-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex-1 p-4 lg:p-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            <div className="px-6 py-4 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-600">Update the details below to modify your product</p>
            </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                    ...
                    {existingImages.length > 0 && (
                      <div className="mb-6">
                        <label className="block text-sm font-semibold text-gray-700 mb-4 flex items-center space-x-2">
                          ...
                          <span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded-full">{existingImages.length} images</span>
                        </label>
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                          {existingImages.map((imageUrl, index) => (
                               ...
                               <button
                                 ...
                                 className="absolute top-2 right-2 z-30 bg-red-500 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm hover:bg-red-600 transition-colors duration-200 shadow-lg border-2 border-white"
                                 disabled={isSubmitting}
                                 title="Remove image"
                               >
                               ...
                               <div className="aspect-square rounded-xl overflow-hidden border-2 border-gray-300 bg-gray-100">
                                                                                                  <img
                                   src={imageUrl}
                                   alt={`Product ${index + 1}`}
                                   ...
                                   onError={(e) => {
                                     console.error(`Failed to load existing image ${index}:`, imageUrl);
                                   ...
                                   onLoad={() => {
                                     console.log(`Existing image ${index} loaded successfully:`, imageUrl);
                                   }}
                                 />
                                <div className={`w-full h-full flex items-center justify-center bg-gray-200 hidden`}>
                                 ...
                                 <button
                                   type="button"
                                   onClick={() => removeExistingImage(index)}
                                   className="opacity-0 group-hover:opacity-100 transition-opacity bg-red-500 text-white px-3 py-2 rounded-lg text-sm font-medium hover:bg-red-600 shadow-lg border-2 border-white"
                                   disabled={isSubmitting}
                                   style={{ 
                                     zIndex: 20,
                                     position: 'relative'
                    ...
                    <div className="mb-4">
                      <label className="block text-sm font-semibold text-gray-700 mb-4 flex items-center space-x-2">
                        ...
                        <span className="text-red-500">*</span>
                        <span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded-full">Up to 4 total</span>
                      </label>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-6">
                      {[0, 1, 2, 3].map((index) => {
                        const existingImage = existingImages[index] || undefined;
                        console.log(`ImageUploadWithCrop ${index} - existingImage:`, existingImage);
                        return (
                          <ImageUploadWithCrop
                            key={index}
                      ...
                      <div className="mt-6 bg-white rounded-xl p-4 border border-purple-200">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className="text-sm text-purple-600 font-semibold">
                            New images: {newImages.filter(img => img !== null).length} / 4
                          </span>
                        </div>
                        
                        <div className="space-y-3">
                          <p className="text-sm font-medium text-gray-700">New Image Previews:</p>
                              ...
                              image && (
                                <div key={index} className="relative group">
                                  <img
                                    ...
                                    className="h-20 w-20 object-cover rounded-lg border-2 border-purple-200 shadow-sm"
                                  />
                                  <button
                                    type="button"
                                    onClick={() => handleImageChange(index, null)}
                                    className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs hover:bg-red-600 transition-colors duration-200 shadow-md"
                                  >
                                    ×
                                  </button>
                                </div>
